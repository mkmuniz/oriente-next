...
const Item: any = styled.div`
  ...
  padding: 10px 0 10px 0;
  text-align: center;
  border-bottom: 1px solid ${(props) => props.theme.cores.branco};
  cursor: pointer;
  :hover {
    transition: 0.5s;
...
export const HomeCoord = ({ children }: any) => {
  const router = useRouter();
  const [menuPosition, setPosition]: any = useState("-100%");
  const data = new Date();
    ...
    >
      <MenuDiv left={menuPosition}>
        <Menu>
          ...
          <Item onClick={() => router.push("/coordenador")}>Home</Item>

          <Item onClick={() => router.push("/tabela2")}>Alunos</Item>

          <Item onClick={() => router.push("/tabelaProf")}>Professores</Item>

          <Item onClick={() => router.push("/projetoCoord")}>Projetos</Item>

          <Item onClick={() => router.push("")}>Avaliações</Item>

          <Item onClick={() => router.push("/sessoes")}>Sessões</Item>

          <Item onClick={() => router.push("/relatorios")}>Relatórios</Item>
