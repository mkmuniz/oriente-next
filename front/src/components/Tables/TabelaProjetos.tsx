...
export const TabelaProjetos = ({ mudaVisibilidade }: any) => {
  ...
  const filteredAlunos = projetos.filter((item: any) => {
      ...
      try {
        const route =
          sessionStorage.getItem("permissao") === "professor"
            ? "/professor/projetos/orientador"
            : "/projeto";
        const [projetos, response] = await Promise.all([
          axios.get(route),
          axios.get("/permissoes"),
        ]);

        if (projetos.status === 200 && response.status === 200) {
          const perm = response.data[0];
